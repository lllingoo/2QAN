...
class QuRouter(BenchArch):
    ...
    **References:**
    ...
    def run_qaoa(self, bench, layers, gammas, betas, msmt='False', schedule=True, 
                    scheduled_instrs=None, init_map=None, maps=None):
        self.router()
        if schedule:
            ordered_all_instrs, dressed_instrs, ordered_all_instrs_phy = self.scheduler()
            init_map = self.init_map
            maps = self.maps
        else:
            ordered_all_instrs, dressed_instrs, ordered_all_instrs_phy = scheduled_instrs
